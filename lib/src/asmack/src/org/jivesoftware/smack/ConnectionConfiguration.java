...
public class ConnectionConfiguration implements Cloneable {
    ...
    public ConnectionConfiguration(String serviceName) {
        ...
        DNSUtil.HostAddress[] addresses = DNSUtil.resolveXMPPDomain(serviceName);
        init(addresses, serviceName,
    ...
    public ConnectionConfiguration(String serviceName,ProxyInfo proxy) {
        // Perform DNS lookup to get host and port to use
        HostAddress[] addresses = DNSUtil.resolveXMPPDomain(serviceName);
        init(addresses, serviceName, proxy);
    }
    ...
    private void init(String host, int port, String serviceName, ProxyInfo proxy) {
        init(new HostAddress[] { new HostAddress(host, port) }, serviceName, proxy);
    }

    private void init(HostAddress[] hosts, String serviceName, ProxyInfo proxy) {
        this.hosts = hosts;
    ...
    /**
     * Returns the possible host addresses to use when establishing the connection. The host addresses to use
    ...
    * Returns the host to use when establishing the connection. The host and port to use
    ...
    public String getHost() {
        return usedHost == null ? null : usedHost.getHost();
    }
     ...
     */
    public int getPort() {
        return usedHost == null ? -1 : usedHost.getPort();
    }
