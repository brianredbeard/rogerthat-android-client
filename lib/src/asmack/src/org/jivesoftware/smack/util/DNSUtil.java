...
public class DNSUtil {
	...
	private static HostAddress[] resolveSRV(String domain) {
	    SortedSet<SRVRecord> records = new TreeSet<SRVRecord>(new SRVRecordComparator());
	    try {
        ...
        HostAddress[] hosts = new HostAddress[records.size()];
        ...
        for (SRVRecord record : records) {
            ...
            hosts[i] = new HostAddress(host, record.getPort());
            i++;
        }
	...
	public static HostAddress[] resolveXMPPDomain(String domain) {
		...
		synchronized (ccache) {
			if (ccache.containsKey(domain)) {
				HostAddress[] addresses = (HostAddress[]) ccache.get(domain);
				if (addresses != null) {
				    for (HostAddress hostAddress : addresses)
                        hostAddress.setFromCache(true);
                    return addresses;
				}
			}
		}
...
//		}
		if (result == null || result.length == 0) {
		    result = new HostAddress[] { new HostAddress(domain, 5222) };
		} else {
    		// Add item to cache.
    		synchronized (ccache) {
	...
	public static HostAddress[] resolveXMPPServerDomain(String domain) {
		...
		synchronized (scache) {
			if (scache.containsKey(domain)) {
				HostAddress[] addresses = (HostAddress[]) scache.get(domain);
				if (addresses != null) {
                    for (HostAddress hostAddress : addresses)
                        hostAddress.setFromCache(true);
					return addresses;
...
//		}
		if (result == null || result.length == 0) {
			result = new HostAddress[] { new HostAddress(domain, 5269) };
		} else {
    		// Add item to cache.
    		synchronized (scache) {
	...
	public static class HostAddress {
		...
		public String toString() {
			return host + ":" + port + (fromCache ? " (from cache)" : "");
		}
