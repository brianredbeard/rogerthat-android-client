...
public class VerifyPaymentAssetResponseHandler extends ResponseHandler<VerifyPaymentAssetResponseTO> {
    ...
    @Override
    public void handle(final IResponse<VerifyPaymentAssetResponseTO> result) {
        T.BIZZ();
        VerifyPaymentAssetResponseTO response;
        try {
            response = result.getResponse();
        } catch (Exception e) {
            L.d("Verify payment asset api call failed", e);
            Intent intent = new Intent(PaymentPlugin.VERIFY_PAYMENT_ASSET_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
            return;
        }

        Intent intent = new Intent(PaymentPlugin.VERIFY_PAYMENT_ASSET_RESULT_INTENT);
