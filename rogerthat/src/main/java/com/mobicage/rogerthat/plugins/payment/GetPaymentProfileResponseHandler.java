...
public class GetPaymentProfileResponseHandler extends ResponseHandler<GetPaymentProfileResponseTO> {
    ...
    @Override
    public void handle(final IResponse<GetPaymentProfileResponseTO> result) {
        T.BIZZ();
        GetPaymentProfileResponseTO response;
        try {
            response = result.getResponse();
        } catch (Exception e) {
            L.d("Get payment profile api call failed", e);
            Intent intent = new Intent(PaymentPlugin.GET_PAYMENT_PROFILE_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
            return;
        }

        Intent intent = new Intent(PaymentPlugin.GET_PAYMENT_PROFILE_RESULT_INTENT);
