...
public class CreatePaymentAssetResponseHandler extends ResponseHandler<CreateAssetResponseTO> {
    ...
    @Override
    public void handle(final IResponse<CreateAssetResponseTO> result) {
        T.BIZZ();
        try {
            CreateAssetResponseTO response = result.getResponse();
            Intent intent = new Intent(PaymentPlugin.CREATE_PAYMENT_ASSET_RESULT_INTENT);
            intent.putExtra("json", JSONValue.toJSONString(response.toJSONMap()));
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
        } catch (Exception e) {
            L.d("Create asset api call failed", e);
            Intent intent = new Intent(PaymentPlugin.CREATE_PAYMENT_ASSET_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
        }
    }
