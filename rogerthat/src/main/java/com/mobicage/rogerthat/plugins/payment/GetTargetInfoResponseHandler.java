...
public class GetTargetInfoResponseHandler extends ResponseHandler<GetTargetInfoResponseTO> {
    ...
    @Override
    public void handle(final IResponse<GetTargetInfoResponseTO> result) {
        T.BIZZ();
        GetTargetInfoResponseTO response;
        try {
            response = result.getResponse();
        } catch (Exception e) {
            L.d("get target info api call failed", e);
            Intent intent = new Intent(PaymentPlugin.GET_TARGET_INFO_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
            return;
        }

        Intent intent = new Intent(PaymentPlugin.GET_TARGET_INFO_RESULT_INTENT);
