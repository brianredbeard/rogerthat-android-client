...
public class ReceivePaymentResponseHandler extends ResponseHandler<ReceivePaymentResponseTO> {
    ...
    @Override
    public void handle(final IResponse<ReceivePaymentResponseTO> result) {
        T.BIZZ();
        ReceivePaymentResponseTO response;
        try {
            response = result.getResponse();
        } catch (Exception e) {
            L.d("Receive payment api call failed", e);
            Intent intent = new Intent(PaymentPlugin.RECEIVE_PAYMENT_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
            return;
        }

        Intent intent = new Intent(PaymentPlugin.RECEIVE_PAYMENT_RESULT_INTENT);
