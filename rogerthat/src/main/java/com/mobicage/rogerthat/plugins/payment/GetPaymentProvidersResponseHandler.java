...
public class GetPaymentProvidersResponseHandler  extends ResponseHandler<GetPaymentProvidersResponseTO> {
    ...
    @Override
    public void handle(final IResponse<GetPaymentProvidersResponseTO> result) {
        T.BIZZ();
        GetPaymentProvidersResponseTO response;
        try {
            response = result.getResponse();
        } catch (Exception e) {
            L.d("Get payment providers api call failed", e);
            Intent intent = new Intent(PaymentPlugin.GET_PAYMENT_PROVIDERS_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
            return;
        }

        Intent intent = new Intent(PaymentPlugin.GET_PAYMENT_PROVIDERS_RESULT_INTENT);
