...
public class GetPendingPaymentDetailsResponseHandler extends ResponseHandler<GetPendingPaymentDetailsResponseTO> {
    ...
    @Override
    public void handle(final IResponse<GetPendingPaymentDetailsResponseTO> result) {
        T.BIZZ();
        GetPendingPaymentDetailsResponseTO response;
        try {
            response = result.getResponse();
        } catch (Exception e) {
            L.d("get pending payment details api call failed", e);
            Intent intent = new Intent(PaymentPlugin.GET_PENDING_PAYMENT_DETAILS_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
            return;
        }

        Intent intent = new Intent(PaymentPlugin.GET_PENDING_PAYMENT_DETAILS_RESULT_INTENT);
