...
public class GetPendingPaymentSignatureDataResponseHandler extends ResponseHandler<GetPendingPaymentSignatureDataResponseTO> {
    ...
    @Override
    public void handle(final IResponse<GetPendingPaymentSignatureDataResponseTO> result) {
        T.BIZZ();
        GetPendingPaymentSignatureDataResponseTO response;
        try {
            response = result.getResponse();
        } catch (Exception e) {
            L.d("get pending payment signature data api call failed", e);
            Intent intent = new Intent(PaymentPlugin.GET_PENDING_PAYMENT_SIGNATURE_DATA_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
            return;
        }

        Intent intent = new Intent(PaymentPlugin.GET_PENDING_PAYMENT_SIGNATURE_DATA_RESULT_INTENT);
