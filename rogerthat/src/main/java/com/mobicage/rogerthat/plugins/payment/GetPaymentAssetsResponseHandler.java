...
public class GetPaymentAssetsResponseHandler extends ResponseHandler<GetPaymentAssetsResponseTO> {
    ...
    @Override
    public void handle(final IResponse<GetPaymentAssetsResponseTO> result) {
        T.BIZZ();
        GetPaymentAssetsResponseTO response;
        try {
            response = result.getResponse();
        } catch (Exception e) {
            L.d("Get payment assets api call failed", e);
            Intent intent = new Intent(PaymentPlugin.GET_PAYMENT_ASSETS_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
            return;
        }

        Intent intent = new Intent(PaymentPlugin.GET_PAYMENT_ASSETS_RESULT_INTENT);
