...
public class ConfirmPaymentResponseHandler extends ResponseHandler<ConfirmPaymentResponseTO> {
    ...
    @Override
    public void handle(final IResponse<ConfirmPaymentResponseTO> result) {
        T.BIZZ();
        ConfirmPaymentResponseTO response;
        try {
            response = result.getResponse();
        } catch (Exception e) {
            L.d("Confirm payment api call failed", e);
            Intent intent = new Intent(PaymentPlugin.CONFIRM_PAYMENT_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
            return;
        }

        Intent intent = new Intent(PaymentPlugin.CONFIRM_PAYMENT_RESULT_INTENT);
