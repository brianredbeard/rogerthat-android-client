...
public class CancelPaymentResponseHandler extends ResponseHandler<CancelPaymentResponseTO> {
    ...
    @Override
    public void handle(final IResponse<CancelPaymentResponseTO> result) {
        T.BIZZ();
        CancelPaymentResponseTO response;
        try {
            response = result.getResponse();
        } catch (Exception e) {
            L.d("Cancel payment api call failed", e);
            Intent intent = new Intent(PaymentPlugin.CANCEL_PAYMENT_FAILED_INTENT);
            intent.putExtra("callback_key", mCallbackKey);
            mMainService.sendBroadcast(intent);
            return;
        }

        Intent intent = new Intent(PaymentPlugin.CANCEL_PAYMENT_RESULT_INTENT);
