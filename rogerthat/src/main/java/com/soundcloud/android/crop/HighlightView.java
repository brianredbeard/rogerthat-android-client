...
class HighlightView {
    ...
    /*
     * Clip path is broken, unreliable or not supported on: - JellyBean MR1 - ICS & ICS MR1 with hardware acceleration
     * turned on
     */
    @SuppressLint("NewApi")
    ...
    private void drawThirds(Canvas canvas) {
        ...
        canvas.drawLine(drawRect.left + xThird * 2, drawRect.top, drawRect.left + xThird * 2, drawRect.bottom,
            outlinePaint);
        canvas.drawLine(drawRect.left, drawRect.top + yThird, drawRect.right, drawRect.top + yThird, outlinePaint);
        canvas.drawLine(drawRect.left, drawRect.top + yThird * 2, drawRect.right, drawRect.top + yThird * 2,
            outlinePaint);
    }
    ...
    public Rect getScaledCropRect(float scale) {
        return new Rect((int) (cropRect.left * scale), (int) (cropRect.top * scale), (int) (cropRect.right * scale),
