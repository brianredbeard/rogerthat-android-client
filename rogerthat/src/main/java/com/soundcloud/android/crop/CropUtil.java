...
public class CropUtil {
    ...
    public static boolean copyExifRotation(File sourceFile, File destFile) {
        ...
        try {
            ...
            exifDest
                .setAttribute(ExifInterface.TAG_ORIENTATION, exifSource.getAttribute(ExifInterface.TAG_ORIENTATION));
    ...
    public static int getExifRotation(File imageFile) {
        if (imageFile == null)
            return 0;
        return CropUtil.getExifRotation(imageFile.getAbsolutePath());
    }
    ...
    public static boolean saveExifRotation(File file, int exifRotation) {
        if (file == null)
            return false;
        return CropUtil.saveExifRotation(file.getAbsolutePath(), exifRotation);
    }

    public static boolean saveExifRotation(String file, int exifRotation) {
        if (file == null)
            return false;
        try {
            ExifInterface exifDest = new ExifInterface(file);
            ...
            exifDest.setAttribute(ExifInterface.TAG_ORIENTATION, String.valueOf(exif_orientation));
            exifDest.saveAttributes();
            return true;
        } catch (IOException e) {
            L.e("Error saving Exif rotation data", e);
            return false;
        }
    }
        ...
        if (SCHEME_FILE.equals(uri.getScheme())) {
            ...
            try {
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    final int columnIndex = (uri.toString().startsWith("content://com.google.android.gallery3d")) ? cursor
                        .getColumnIndex(MediaStore.MediaColumns.DISPLAY_NAME) : cursor
