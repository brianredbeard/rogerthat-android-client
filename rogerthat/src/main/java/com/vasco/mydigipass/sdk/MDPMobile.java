...
public class MDPMobile {
    ...
    /**
     * Default constructor. Provide the activity where you are going to implement the MYDIGIPASS login button.
     *
     * @param activity
     *            Activity that implements a MYDIGIPASS login button.
     */
    ...
    /**
     * Constructor Provide your MYDIGIPASS client id and mobile redirect uri.
     *
     * @param activity
     *            Activity that implements a MYDIGIPASS login button.
     * @param clientId
     *            Your MYDIGIPASS client ID registered via developer.mydigipass.com.
     * @param redirectUri
     *            Your MYDIGIPASS mobile redirect uri registered via developer.mydigipass.com.
     */
     ...
     *
     * @param clientId
     *            Your MYDIGIPASS client ID registered via https://developer.mydigipass.com.
     */
    ...
    /**
     * Returns the OAuth state parameter set in the original authentication request. See
     * http://tools.ietf.org/html/rfc6749#section-10.12 for more information.
     *
     * @return state
     */
     ...
     *
     * @param mdpAuthenticationListener
     *            Class that implements the OnMDPAuthenticationListener.
     * @see com.vasco.mydigipass.sdk.OnMDPAuthenticationListener
     */
    ...
    /**
     * Perform an authenticate request. If the app exists on the user's phone, it is going to open the app. If the app
     * is not installed a dialog will ask to install it or to perform the authentication in a mobile browser instead.
     *
     * @param state
     *            parameter to validate your request.
     * @throws MDPException
     */
     ...
     *
     * @param state
     *            An opaque value used by the client to maintain state between the request and callback, not null
     * @param scope
     *            Space-delimited values used to determine which MYDIGIPASS user data can be consumed with the OAuth
     *            Access Token, may be null
     * @param passthroughParams
     *            Anything extra, may be null
     */
    ...
    /**
     * Lifecycle call. Call it in your onCreate of your activity. It enables authentication via the user's browser when
     * the MYDIGIPASS Authenticator For Mobile app is not installed. If it's not an authentication request, this method
     * does nothing.
     */
    public void webFlow() {
                ...
                if (data.toString() != null && isValidResponseRedirectUri(data.toString())) {
                    try {
                        ...
                        if (code != null && state != null) {
                            response = buildResponse(true, code, state, getRedirectUri(), getScope(),
                                getPassthroughParams());
                            mOnAuthenticatedListener.onMDPAuthenticationSuccess(response);
                        } else {
                            response = buildResponse(false, code, state, getRedirectUri(), getScope(),
                                getPassthroughParams());
                            mOnAuthenticatedListener.onMDPAuthenticationFail(response);
                        }

                    } catch (UnsupportedOperationException e) {
                        mOnAuthenticatedListener.onMDPAuthenticationFail(buildResponse(false, null, null,
                            getRedirectUri(), getScope(), getPassthroughParams()));
    ...
    /**
     * Lifecycle call. Override the onActivityResult method of your activity and call this method. It handles the intent
     * from the MYDIGIPASS app and does the callbacks on the interface.
     *
     * @param requestCode
     *            requestCode
     * @param resultCode
     *            resultCode
     * @param data
     *            Intent from the MYDIGIPASS app
     */
     ...
     *
     * @param data
     *            data intent coming from the onActivityResult method.
     ...
     *            authorization code
     * @param state
     *            user's state parameter
     * @param redirectUri
     *            redirectUri.
     * @return MDPResponse
     */
    private MDPResponse buildResponse(boolean success, String code, String state, String redirectUri, String scope,
    ...
    private AlertDialog buildDialog() {
        ...
        if (isMdpInstalled()) {
            builder.setMessage(context.getString(R.string.update_app_text)).setTitle(
                context.getString(R.string.title_dialog));
            builder.setPositiveButton(context.getString(R.string.update_app), new OkButtonListener(this));
        } else {
            builder.setMessage(context.getString(R.string.install_app_text)).setTitle(
                context.getString(R.string.title_dialog));
